CASESystem.plyStoreInterface = function()
	...
	if CASESystem.Config.AnimateUISlide == true then 
	...
	frame.Paint = function( self, w, h )
		...
		draw.RoundedBox(0, 0, 0, w, h, Color(167, 221, 248))
	...
	for b, n in ipairs(CASESystem.Config.ShopTabs) do

		-- Core tab panel
		local buttonpanel = vgui.Create("DButton", buttonscont)
		buttonpanel:SetSize(buttonscont:GetWide()/#CASESystem.Config.ShopTabs, buttonscont:GetTall())
		buttonpanel:SetText("")
		...
		buttonpanel.DoClick = function()
			...
			clistcont:Clear()
			for k, v in ipairs(CASESystem.Config.ShopContent) do
				if n.display == v.tab then

					if not CASESystem.Config.ShowUnpurchasable then
					...
					if LocalPlayer():getDarkRPVar("money") >= v.price then canaffordcolor = CASESystem.Config.ShopBuyColor else canaffordcolor = CASESystem.Config.ShopBuyDenyColor end
					button.Paint = function(self,w,h)
						draw.RoundedBox(4, 0, 0, w, h, canaffordcolor)
						draw.DrawText(CASESystem.Language.Buy, "npcrob30", w/2, 0, textwhite, TEXT_ALIGN_CENTER)
						draw.DrawText("$"..CASESystem.Core.FormatFunc(v.price), "npcrob25", w/2, h-25, textwhite, TEXT_ALIGN_CENTER)
					end
					-- Send a request to the server to buy the item
					button.DoClick = function()
						if !CASESystem.Config.KeepUIOpen then
							frame:Close()
						end
						local TABLE = {}
						TABLE.ent = v.ent
					...
					if CASESystem.Config.ShopModelRotate == false then
		...
		-- The initial spawn of the list
	for b, n in pairs(CASESystem.Config.ShopTabs) do
		if b == 1 then
			currentselected = n.display
		end
	end
