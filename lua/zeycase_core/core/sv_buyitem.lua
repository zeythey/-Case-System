...
net.Receive("ncpstoredoact", function(len, pl)
	...
	for k, v in pairs(CASESystem.Config.ShopContent) do
	...
	if buyer:getDarkRPVar("money") < ItemExists.price then
		net.Start( "ncpshopmsg" )
			net.WriteString( string.format( CASESystem.Language.NoBuyMoney, ItemExists.name ) )
		net.Send(buyer)
		return
	end
	...
	if ItemExists.customCheck ~= nil then
		if !(ItemExists.customCheck(buyer)) then
			net.Start( "ncpshopmsg" )
				net.WriteString( string.format( CASESystem.Language.NoBuyRank, ItemExists.name ) )
			net.Send(buyer)
			return
		end
	end
	...
	if ItemExists.isWep then
		...
		net.Start( "ncpshopmsg" )
			net.WriteString( string.format( CASESystem.Language.Brought, ItemExists.name, number1000(ItemExists.price) ) )
		net.Send(buyer)
		return
	end
